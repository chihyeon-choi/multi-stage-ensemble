...
from util_relabel import evaluate_final_model, loss_separation
...
parser.add_argument('--exp_name', type=str, default="animal_test")
...
# Dataset load
total_data_loader = dataset.Animal10NDatasetLoader(data_dir='../datasets/Animal10N/raw_image', num_of_workers=0).getDataLoader()
data_loader = total_data_loader['train_dataset']

test_data_loader = total_data_loader['test_dataset']
class_num = 10
...
def main():
    ...
    for stage in range(args.n_stage):
        ...
        criterion = loss.NCEandRCE(alpha=args.alpha,beta=args.beta,num_classes=10)
        ...
        if stage < args.n_stage - 1:
            ...
            sorted_passive_loss_list, sorted_passive_ind_list = sort_loss_per_class(model, data_loader, criterion, class_num)
            ...
            relabel_info = relabel_criterion(model, data_loader, indexes_dict, mean_vector_list, args.confidence_threshold, class_num, logger)
            # Refine dataset by re-labeling
            data_loader = relabeling_dataset(data_loader, relabel_info)
        else:
    ...
    total = 0
    prediction_dict = {}
    for images, labels, indexes, img_path in test_data_loader:    
