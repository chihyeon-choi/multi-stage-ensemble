...
class ToyModel(nn.Module):
    def __init__(self, type='CIFAR10'):
        ...
        if type == 'CIFAR10':
            ...
            # self.global_avg_pool = nn.AdaptiveAvgPool2d(1)
            self.fc1 = nn.Sequential(
                nn.Linear(3*3*196, 256),
                nn.BatchNorm1d(256),
                nn.ReLU())
            self.fc2 = nn.Linear(256, 10)
            self.fc_size = 3*3*196            
        elif type == 'MNIST':
            self.block1 = nn.Sequential(
                ConvBrunch(1, 32, 3),
                nn.MaxPool2d(kernel_size=2, stride=2))
    ...
    #     x1 = self.fc1(x)
    #     # x1 shape : [batchsize, 128]
    #     x2 = self.fc2(x1)
    #     # x2 shape : [batchsize, 10]
    #     return x2, x1
    def forward(self, x):
        # print(x.shape)
        x = self.block1(x)
        ...
        x = self.block2(x)
        # print(x.shape)
        if self.type == 'CIFAR10' or self.type == 'FMNIST':
            x = self.block3(x)  
            # print(x.shape)
        # x = self.global_avg_pool(x)
        # x = x.view(x.shape[0], -1)
...
def ResNet34(in_channel, num_classes=10):
    return ResNet(BasicBlock, [3, 4, 6, 3], in_channel, num_classes=num_classes)
