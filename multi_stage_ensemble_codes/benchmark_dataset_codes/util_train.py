...
def validate(model, data_loader, criterion, global_step, epoch, stage, logger):
    ...
    model.eval()
    with torch.no_grad():
        for images, labels, indexes in data_loader['valid_dataset']:
            images, labels = images.to(device, non_blocking=True), labels.to(device, non_blocking=True)
...
def train_epoch(model, data_loader, optimizer, ENV, epoch, criterion, stage, logger, config):
    ...
    for i, (images, labels, indexes) in enumerate(data_loader['train_dataset']):
        ...
        # true_labels = np.array([total_true_label[i] for i in ind])
...
def train(model, optimizer, scheduler, criterion, data_loader, ENV, stage, logger, config, save_path):
    ...
    for epoch in range(config.epochs):
        ...
        valid_acc1, valid_loss_1 = validate(model, data_loader, torch.nn.CrossEntropyLoss(), ENV['global_step'], epoch, stage, logger)
        
        if valid_acc1 > best_acc:
            print('model saved, valid acc {} ==> {}'.format(best_acc, valid_acc1))
            best_acc = valid_acc1
        ...
        test_acc1, test_loss_1 = evaluate(model, data_loader, torch.nn.CrossEntropyLoss(), ENV['global_step'], epoch, stage, logger)
    ...
    print('for check, best validation acc',best_acc) 
