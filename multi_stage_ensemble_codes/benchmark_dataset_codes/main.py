...
def main():
    ...
    for stage in range(args.n_stage):
        ...
        if stage < args.n_stage - 1:
            ...
            sorted_passive_loss_list, sorted_passive_ind_list = sort_loss_per_class(model, data_loader, total_true_label, criterion)
            # Separate small-loss dataset and large-loss dataset 
            indexes_dict, reference_indexes, ratio_criteria = loss_separation(args.small_loss_ratio, args.relabel_threshold, sorted_passive_loss_list, sorted_passive_ind_list, clean_or_not, logger)
            # Compute mean feature vector for each class
            mean_vector_list = mean_vector_per_class(model, data_loader, reference_indexes, clean_or_not, indexes_dict, logger)
            # Compute re-labeling confidence value on the each large loss data
            relabel_info = relabel_critetion(model, data_loader, indexes_dict, mean_vector_list, total_true_label, args.confidence_threshold, logger)
            # Refine dataset by re-labeling
            data_loader, clean_or_not = relabeling_dataset(data_loader, relabel_info, clean_or_not)
            # Check the re-labeling accuracy
            relabel_acc(relabel_info, num_class_list, ratio_clean_list, logger)
        else:
            num_clean_list, ratio_clean_list, num_class_list = compute_noise_rate(total_true_label, total_noisy_label, logger)
    ...
    for images, labels in data_loader['test_dataset']:    
        ...
        with torch.no_grad():
            ...
            for i in range(labels.size(0)):
                voting_list = [0 for n in range(class_num)]
                confidence_list = [0 for n in range(class_num)]
