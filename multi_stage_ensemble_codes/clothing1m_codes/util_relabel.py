...
def get_label_info(noisy_loader, logger):
    
    noisy_labels = [0 for i in range(14)]
...
def sort_loss_per_class(model, data_loader, criterion):
    ...
    with torch.no_grad():
        for i, (images, labels, indexes, img_path) in enumerate(data_loader):
            ...
            for j in range(len(loss)):
                ...
                image_path_dict[ind[j]] = img_path[j]
    ...
    return sorted_passive_loss_list, sorted_passive_ind_list, image_path_dict
...
def relabel_criterion(model, data_loader, indexes_dict, mean_vector_list, relabel_threshold, logger):
...
def relabeling_dataset(data_loader, relabel_info, image_path_dict):
    # relabel_info : [[index, original_label, re_label], ...]
...
#     data_loader.train_dataset.train_labels[k] = 1
    ...
    for i in range(len(relabel_info)):
        ...
        data_loader.train_dataset.train_labels[image_path_dict[temp_index]] = temp_relabeled
